...
void SetDeskTopNum( char *line, FILE *fp )
{
	...
	if( Scr.LastActive!=NULL )
		free( Scr.LastActive );
	Scr.LastActive = calloc( Scr.n_desktop, sizeof(OxwmWindow));
}
...
void SetBalloonHelp( char *line, FILE *fp )
{
	...
	free( Scr.BalloonOffStr );
	free( Scr.BalloonOnStr );
...
void ReadConfigFile( char *configfile )
{
  ...
  size_t len;
	...
	size_t sysrc_size;
#endif
	int is_default = (strcmp( configfile, CONFIGNAME ) == 0);
	...
	if( !file ){
		if( !is_default ){
			sysrc_size = strlen(OXWMLIBDIR)+strlen(configfile)+2;
			sysrc = calloc( sysrc_size, 1 );
			snprintf( sysrc, sysrc_size, "%s/%s", OXWMLIBDIR, configfile );
		}
		else{
			sysrc_size = strlen(OXWMLIBDIR)+strlen(configfile)+9;
			sysrc = calloc( sysrc_size, 1 );
			snprintf( sysrc, sysrc_size, "%s/system%s", OXWMLIBDIR, configfile );
		}
		file = LookUpFiles( NULL, sysrc, R_OK );
		free( sysrc );
	}
#endif
	/* Fallback to legacy MLVWM rc file name for back-compat with
	 * configs from the original MLVWM.  Only applies when the user did
	 * not explicitly pass -f. */
	if( !file && is_default ){
		const char *legacy = ".mlvwmrc";
		if( (file = LookUpFiles( NULL, legacy, R_OK ))==NULL )
			file = LookUpFiles( getenv("HOME"), legacy, R_OK );
#ifdef OXWMLIBDIR
		if( !file ){
			sysrc_size = strlen(OXWMLIBDIR)+strlen(legacy)+9;
			sysrc = calloc( sysrc_size, 1 );
			snprintf( sysrc, sysrc_size, "%s/system%s", OXWMLIBDIR, legacy );
			file = LookUpFiles( NULL, sysrc, R_OK );
			free( sysrc );
		}
#endif
