...
void IconZoom ( OxwmWindow *mw, Bool on )
{
...
void ShadeWindow( OxwmWindow *mw )
{
	...
	if( mw->flags&SHADER ){
		attr.cursor = Scr.OxwmCursors[SHADER_DOWN_CURSOR];
		valuemask = CWCursor;
...
void UnShadeWindow( OxwmWindow *mw )
{
	unsigned long valuemask;
	XSetWindowAttributes attr;
	...
	if( mw->flags&SHADER ){
		attr.cursor = Scr.OxwmCursors[SHADER_UP_CURSOR];
		valuemask = CWCursor;
		XChangeWindowAttributes( dpy, mw->shade_b, valuemask, &attr );
	}
...
OxwmWindow *ChoiseWindow( XEvent *ev, int cursor )
{
	Bool loop=True;
	OxwmWindow *tmp_win;
	Window select_win;
	...
	if(XFindContext(dpy, select_win, OxwmContext,	(caddr_t *)&tmp_win)==XCNOENT)
		tmp_win = NULL;
	return tmp_win;
}
...
void ChangeDesk( char *action )
{
	...
	{
		char deskmsg[32];
		snprintf( deskmsg, sizeof(deskmsg), "Desk %d", newdesk );
		DrawStringMenuBar( deskmsg );
	}
...
void SelectWindow( char *action )
{
	...
	if( tmp->flags&HIDED ){
		for( testwin = (OxwmWindow *)Scr.OxwmRoot.next; testwin != NULL;
			testwin = (OxwmWindow *)testwin->next)
...
{
	OxwmWindow *tmp, *last, *start, *lastactive=NULL;
	char desk[8];
	int newdesk, lastdesk=0;
	Bool samedesk=False;

	if( !Scr.OxwmRoot.next )	return;

	if( !strcmp( action, "NextSameDeskWindow" ))		samedesk = True;

	for( last = Scr.OxwmRoot.next; last && last->next!=NULL;
		last = last->next );

	if( Scr.ActiveWin==NULL || Scr.ActiveWin->next==NULL ){
		tmp = Scr.OxwmRoot.next;
		start = last;
	}
	else{
	...
	while( (tmp->flags&HIDED || tmp->flags&SKIPSELECT ||
		...
		tmp = tmp->next;
		if( tmp==NULL )		tmp = Scr.OxwmRoot.next;
	}
...
void SelectPreviousWindow( char *action )
{
	...
	if( Scr.ActiveWin==NULL || Scr.ActiveWin->prev==&Scr.OxwmRoot ){
		tmp = last;
		start = Scr.OxwmRoot.next;
	}
	else{
	...
	do{
		...
		if( Scr.flags&STARTING && ev.type==MapNotify &&
		   XFindContext( dpy, ev.xany.window,
						OxwmContext, (caddr_t *)&tmp_win )!=XCNOENT)
...
void DebugWinList( char *action )
{
	...
	printf("Start\n");
	for( tmp = Scr.OxwmRoot.next; tmp!=NULL; tmp = tmp->next ){
...
void ToggleFullScreen( char *action )
{
	OxwmWindow *mw = Scr.ActiveWin;
	if( !mw ) return;

	if( mw->flags & FULLSCREEN ){
		...
		mw->frame_h = Scr.MyDisplayHeight;
		SetUpFrame( mw, mw->frame_x, mw->frame_y,
			mw->frame_w, mw->frame_h, True );
		mw->flags |= FULLSCREEN;
	}
