    ...
    if(Scr.flags&ROUNDEDCORNERS){
        ...
        if( Scr.flags&SYSTEM8 && !(Scr.flags&STARTING) ){
            XDrawArc( dpy, Scr.MenuBar, Scr.WhiteGC, 0, 0,
                     11, 11, 180*64, -(90*64) );
            XDrawArc( dpy, Scr.MenuBar, Scr.Gray1GC, Scr.MyDisplayWidth-12, 0,
                     11, 11, 0, 90*64 );
            XFillArc( dpy, Scr.MenuBar, Scr.MenuBlueGC, 0, 0,
                     11, 11, 180*64, -(90*64) );
            XFillArc( dpy, Scr.MenuBar, Scr.MenuBlueGC, Scr.MyDisplayWidth-12, 0,
                     11, 11, 0, 90*64 );
        }
        else{
            XFillArc( dpy, Scr.MenuBar, Scr.WhiteGC, 0, 0,
                     11, 11, 180*64, -(90*64) );
            XFillArc( dpy, Scr.MenuBar, Scr.WhiteGC, Scr.MyDisplayWidth-12, 0,
                     11, 11, 0, 90*64 );
        }
...
MenuLabel *DrawMenuItem( MenuLabel *ml, int sel, Bool on )
{
	...
	label_p = ml->ItemHeight==0?14:14+ml->ItemHeight+4;
	...
	if( mi->xpm ){
		...
		if( mi->xpm->kind==PIXMAP ){
			...
			XSetClipOrigin( dpy, s_tmpGC, 
						   14+(ml->IconWidth-mi->xpm->width)/2,
...
Bool MapMenu( MenuLabel *m, int left, int right, int y, Bool side )
{
	...
	for( mi = m->m_item; mi; mi=mi->next ){
		...
		if( mi->xpm ){
			...
			local_mw = 14+mi->xpm->width+4+label_w+14;
		}
		else
			local_mw = 14+label_w+14;
...
Bool SwallowMenu( MenuLabel *ml )
{
	...
	do{
		...
		if( ev.type==MapNotify &&
		   XFindContext(dpy, ev.xany.window, OxwmContext, (caddr_t *)&tmp_win)
		   !=XCNOENT)
...
void MapMenuBar( OxwmWindow *win )
{
	int left, right;
	MenuLabel *tmpml;
	MenuLink *mlink;
	...
	left = 0;
	right = Scr.MyDisplayWidth-Scr.IconMenu.LabelWidth-8;
	...
	if( Scr.MenuRoot==NULL ){
		...
		for( tmpml=Scr.MenuLabelRoot; tmpml; tmpml=tmpml->next ){
			if( tmpml->flags&LEFTSIDE ){
				tmpml->LabelX = left;
				left += tmpml->LabelWidth;
			}
			else{
				tmpml->LabelX = right-tmpml->LabelWidth;
				right = tmpml->LabelX;
			}
	...
	for( mlink=style->menubar->link; mlink; mlink=mlink->next ){
		if( mlink->link->flags&LEFTSIDE ){
			...
			left += mlink->link->LabelWidth;
		}
		else{
			mlink->link->LabelX = right-mlink->link->LabelWidth;
