...
void create_resizebox( OxwmWindow *tmp_win )
{
	...
	attributes.cursor = Scr.OxwmCursors[RESIZE];
	...
	XSaveContext( dpy, tmp_win->resize_b, OxwmContext, (caddr_t)tmp_win );
}

void create_scrollbar( OxwmWindow *tmp_win )
{
	unsigned long valuemask;
	XSetWindowAttributes attributes;
	int title_height;
	...
	if( tmp_win->flags&SBARV ){
		attributes.cursor = Scr.OxwmCursors[SBARV_CURSOR];
		...
		for( lp=0; lp<4; lp++ )
			XSaveContext( dpy, tmp_win->scroll_v[lp], OxwmContext, (caddr_t)tmp_win );
	}
	if( tmp_win->flags&SBARH ){
		attributes.cursor = Scr.OxwmCursors[SBARH_CURSOR];
		...
		for( lp=0; lp<4; lp++ )
			XSaveContext(dpy,tmp_win->scroll_h[lp],OxwmContext,(caddr_t)tmp_win);
	}
}

void create_titlebar( OxwmWindow *tmp_win )
{
	unsigned long valuemask;
	XSetWindowAttributes attributes;
	...
	attributes.event_mask = ButtonPressMask | ButtonReleaseMask | ExposureMask;
	attributes.background_pixel = WhitePixel( dpy, Scr.screen );
	attributes.cursor = Scr.OxwmCursors[TITLE_CURSOR];
	...
	if( tmp_win->flags&CLOSER ){
		attributes.cursor = Scr.OxwmCursors[DESTROY];
		tmp_win->close_b = XCreateWindow( dpy, tmp_win->title_w,
										 ...
										 CopyFromParent,
										 valuemask, &attributes );
		XSaveContext( dpy, tmp_win->close_b, OxwmContext, (caddr_t)tmp_win );
	}
	if( tmp_win->flags&MINMAXR ){
		attributes.cursor = Scr.OxwmCursors[SELECT];
		tmp_win->minmax_b = XCreateWindow( dpy, tmp_win->title_w,
										  ...
										  CopyFromParent,
										  valuemask, &attributes );
		XSaveContext( dpy, tmp_win->minmax_b, OxwmContext, (caddr_t)tmp_win );
	}
	if( tmp_win->flags&SHADER ){
		...
		tmp_win->shade_b = XCreateWindow( dpy, tmp_win->title_w,
										 ...
										 CopyFromParent,
										 valuemask, &attributes );
		XSaveContext( dpy, tmp_win->shade_b, OxwmContext, (caddr_t)tmp_win );
	}
 	XMapSubwindows( dpy, tmp_win->title_w );
	XSaveContext( dpy, tmp_win->title_w, OxwmContext, (caddr_t)tmp_win );
}

void FetchWmProtocols (OxwmWindow *tmp)
{
...
void GrabKeys( OxwmWindow *tmp_win )
{
	ShortCut *tmp;
...
void GetWindowSizeHints( OxwmWindow *mw )
{
    long supplied=0;
...
void SmartPlacement( OxwmWindow *t, int width, int height, int *x, int *y )
{
	int temp_h,temp_w;
	int test_x = 0,test_y = 0;
	int loc_ok = False, tw,tx,ty,th;
	OxwmWindow *test_window;
...
OxwmWindow *AddWindow( Window win )
{
	OxwmWindow *tmp_win, *last;
	unsigned long valuemask;
	XSetWindowAttributes attributes;
	int title_height;
	...
	if( Scr.ActiveWin )
		...
		for ( last = (OxwmWindow *)&Scr.OxwmRoot;
			 last->next != NULL;
			 last = last->next);
	if( last->next ){
	...
	last->next = (struct OxwmWindow *)tmp_win;
	tmp_win->prev = last;
	if( tmp_win==(OxwmWindow *)0 )	return NULL;
	tmp_win->flags = 0;
	tmp_win->w = win;
	...
	if ( XGetCommand( dpy, tmp_win->w, &client_argv, &client_argc ) ) {
		XrmParseCommand( &db, table, 2, "oxwm", &client_argc, client_argv );
		status = XrmGetResource( db, "oxwm.desk", "Oxwm.Desk",
								&str_type, &rm_value );
	...
	XSaveContext( dpy, tmp_win->frame, OxwmContext, (caddr_t)tmp_win );
	XSaveContext( dpy, tmp_win->Parent, OxwmContext, (caddr_t)tmp_win );
	XSaveContext( dpy, tmp_win->w, OxwmContext, (caddr_t)tmp_win );
