...
OxwmWindow *ColormapWindow = &Scr.OxwmRoot;
...
void send_clientmessage( Window w, Atom a, Time timestamp)
{
	...
	if( XFindContext ( dpy, w, OxwmContext, (caddr_t *)&tmp_win )
	   == XCNOENT )
		tmp_win = NULL;
...
void InstallWindowColormaps (OxwmWindow *tmp)
{
...
Bool GrabEvent( int cursor )
{
	...
	while((i<1000)&&(val=XGrabPointer(dpy, Scr.Root, True, mask,
								  ...
								  Scr.OxwmCursors[cursor], CurrentTime)!=
					 GrabSuccess)){
		i++;
		sleep_a_little(1000);
    }
...
void RestoreWithdrawnLocation( OxwmWindow *tmp, Bool restart )
{
	XWindowChanges xwc;
	Window root, child;
	int rx, ry;
...
int GetContext( OxwmWindow *t, XEvent *e, Window *w )
{
	int Context;
...
OxwmWindow *NextActiveWin( OxwmWindow *t )
{
	int lp;
	...
	for( lp=nchildren-1; lp>-1; lp-- ){
		if( XFindContext( dpy, children[lp], OxwmContext,
						 (caddr_t *)&NextActive )
		   !=XCNOENT &&
		   NextActive->w!=t->w &&
...
void SetMapStateProp( OxwmWindow *tmp_win, int state)
{
...
int GetMapStateProp( OxwmWindow *tmp_win )
{
	Atom *protocols = NULL;
	Atom atype;
	int aformat, state;
...
void Destroy( OxwmWindow *t )
{
	...
	XDeleteContext( dpy, t->Parent, OxwmContext );
	XDeleteContext( dpy, t->frame, OxwmContext );
	XDeleteContext( dpy, t->w, OxwmContext );
	if( t->flags&CLOSER )
		XDeleteContext( dpy, t->close_b, OxwmContext );
	if( t->flags&MINMAXR )
		XDeleteContext( dpy, t->minmax_b, OxwmContext );
	if( t->flags&SHADER )
		XDeleteContext( dpy, t->shade_b, OxwmContext );
	if( t->flags&TITLE )
		XDeleteContext( dpy, t->title_w, OxwmContext );
	if( t->flags&SBARV )
		for( lp=0; lp<4; lp++ )
			XDeleteContext( dpy, t->scroll_v[lp], OxwmContext );
	if( t->flags&SBARH )
		for( lp=0; lp<4; lp++ )
			XDeleteContext( dpy, t->scroll_h[lp], OxwmContext );
	if( t->flags&RESIZER )
		XDeleteContext( dpy, t->resize_b, OxwmContext );
	if( Scr.n_desktop>1 )
...
/*	w = ev->xany.window;*/
	...
	if( XFindContext( dpy, w, OxwmContext, (caddr_t *)&tmp_win )==XCNOENT )
		return;
...
void handle_configure_request( XEvent *ev )
{
	...
	if (XFindContext (dpy, xcr->window, OxwmContext, (caddr_t *) &tmp_win) ==
		XCNOENT)
		tmp_win = NULL;
	if( !tmp_win ){
		wc.x = xcr->x;
...
void MoveWindow( OxwmWindow *mw, XEvent *evp )
{
	Bool isEnd = False;
...
void CloseWindow( OxwmWindow *mw, XEvent *evp )
{
...
void DrawResizeFrame( int x, int y, int w, int h, OxwmWindow *t )
{
	XSegment lines[10];
	int nl=0;
...
void ConstrainSize( OxwmWindow *tmp_win, int *widthp, int *heightp )
{
...
void ResizeWindow( OxwmWindow *mw, XEvent *evp, Bool s_move )
{
	Bool isEnd = False;
...
void MinMaxWindow( OxwmWindow *mw, XEvent *evp )
{
	Bool isEnd = False, isIn = True;
	XEvent ev;
	...
	if( isIn ){
		...
		if( mw->flags & MAXIMAIZED ){
			...
			mw->frame_w = Scr.MyDisplayWidth;
			mw->frame_h = Scr.MyDisplayHeight - MENUB_H;
			/* Do NOT ConstrainSize on maximize — we want to fill the
			   screen even if the app sets a small PMaxSize hint. */
			mw->flags |= MAXIMAIZED;
		}
		SetUpFrame( mw, mw->frame_x, mw->frame_y,
				   mw->frame_w, mw->frame_h, True );
...
void ShadeBox( OxwmWindow *mw, XEvent *evp )
{
	Bool isEnd = False, isIn = True;
	XEvent ev;
...
***/
void PressSbarAnker( Bool isHbar, OxwmWindow *tmp_win )
{
...
/***
  isHbar  if scroll bar is Horizontal, then true
***/
void PressSbar( Bool isHbar, Window pressedwin, OxwmWindow *tmp_win )
{
...
void HandleMapRequest( Window win )
{
	...
	if(XFindContext(dpy, win, OxwmContext,
