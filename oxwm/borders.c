...
void SetShape( OxwmWindow *tmp_win, int w)
{
  XRectangle rect[7];
...
void SetUpFrame( OxwmWindow *t, int x, int y, int w, int h, Bool sendEvent )
{
	XWindowChanges xwcv;
	unsigned long xwcm;
...
void DrawCloseBox( OxwmWindow *t, Bool on )
{
	...
	if( on ) attributes_box.cursor = Scr.OxwmCursors[DESTROY];
	else attributes_box.cursor = Scr.OxwmCursors[DEFAULT];
...
void DrawMinMax( OxwmWindow *t, Bool on )
{
	unsigned long mask, valuemask;
	XSetWindowAttributes attributes_box;
	XGCValues xgcv;
	...
	if( on ) attributes_box.cursor = Scr.OxwmCursors[MINMAX_CURSOR];
	else attributes_box.cursor = Scr.OxwmCursors[DEFAULT];
	valuemask = CWBackPixel | CWCursor;
...
void DrawShadeR( OxwmWindow *t, Bool on )
{
	unsigned long mask, valuemask;
	XSetWindowAttributes attributes_box;
	XGCValues xgcv;
	...
	if( on ){
		if( t->flags&SHADE )
			attributes_box.cursor = Scr.OxwmCursors[SHADER_DOWN_CURSOR];
		else
			attributes_box.cursor = Scr.OxwmCursors[SHADER_UP_CURSOR];
	}
	else attributes_box.cursor = Scr.OxwmCursors[DEFAULT];
	valuemask = CWBackPixel | CWCursor;
...
void DrawTitleBarCore( Window w, int fw, int th, const char *title, int active )
{
	...
	if( active ){
		if( Scr.d_depth>1 && Scr.flags&SYSTEM8 ){
			for( lp=4; lp<16; lp+=2 ){
				DrawShadowBox( 4, lp-1, fw-14, 2, w, 1,
								  Scr.WhiteGC, Scr.Gray1GC, SHADOW_ALL );
			}

			DrawShadowBox( 0, 0, fw-2, th, w,
						  1, Scr.WhiteGC, Scr.Gray2GC, SHADOW_TOP );
			XDrawLine( dpy, w, Scr.Gray2GC,
					  2, th-1, fw-9, th-1 );
		}
		else{
			for( lp=3; lp<15; lp+=2 ){
				if( Scr.d_depth>1 )
					XDrawLine( dpy, w, Scr.Gray1GC, 0, lp, fw, lp );
				else
					XDrawLine( dpy, w, Scr.BlackGC, 0, lp, fw, lp );
			}
		...
		StrWidthHeight( WINDOWFONT, &w_px, NULL, &off_px, t, len );
		XFillRectangle( dpy, w, Scr.Gray4GC,
					   (fw-w_px)/2-5, 1, w_px+10, th-2 );
		dispgc = Scr.BlackGC;
	}
	else{
	...
	StrWidthHeight( WINDOWFONT, &w_px, NULL, &off_px, t, len );
	XDRAWSTRING( dpy, w, WINDOWFONT, dispgc, (fw-w_px)/2,
				th/2-off_px+(Scr.flags&SYSTEM8?0:1), t, len );
}

void SetTitleBar( OxwmWindow *t, Bool on_off )
{
	int w, offset;
	...
	if( on_off ) attributes_title.cursor = Scr.OxwmCursors[TITLE_CURSOR];
	else     attributes_title.cursor = Scr.OxwmCursors[DEFAULT];
	valuemask = CWBackPixel | CWCursor;
...
void DrawSbarAnk( OxwmWindow *t, int context, Bool on_off )
{
	Window	win;
	int size, scale, lp;
...
void DrawSbarArrow( OxwmWindow *t, int context, Bool on_off )
{
	int size;
	unsigned int mask, valuemask;
	Window win;
	...
	case C_SBAR_UP:
		win = t->scroll_v[1];
		size = t->frame_h-t->win_h;
		cursor = Scr.OxwmCursors[SBARV_CURSOR];
		break;
	case C_SBAR_DOWN:
		win = t->scroll_v[2];
		size = t->frame_h-t->win_h;
	...
	case C_SBAR_LEFT:
		win = t->scroll_h[1];
		size = t->frame_w-t->win_w;
		cursor = Scr.OxwmCursors[SBARH_CURSOR];
		break;
	case C_SBAR_RIGHT:
		win = t->scroll_h[2];
		size = t->frame_w-t->win_w;
...
void DrawSbarBar( OxwmWindow *t, int context, Bool on_off )
{
	unsigned long mask, valuemask;
	XSetWindowAttributes attributes;
...
void DrawFrameShadow( OxwmWindow *t, Bool on )
{
	XSegment lines[4];
	int lp;
	...
	if( on ) attributes.cursor = Scr.OxwmCursors[TITLE_CURSOR];
	else attributes.cursor = Scr.OxwmCursors[DEFAULT];
	valuemask = CWCursor;
...
void DrawResizeBox( OxwmWindow *t, Bool on_off )
{
	unsigned long mask, valuemask;
	XSetWindowAttributes attributes;
	XGCValues xgcv;
	...
	if( on_off || Scr.flags&SYSTEM8 ){
		...
		attributes.background_pixel = WhitePixel( dpy, Scr.screen );
	if( on_off ) attributes.cursor = Scr.OxwmCursors[RESIZE];
	else     attributes.cursor = Scr.OxwmCursors[DEFAULT];
	valuemask = CWBackPixel | CWCursor;
...
void DrawAllDecorations( OxwmWindow *t, Bool on_off )
{
	if( t->flags&SBARV ){
...
{
	DrawShadowBox( cx, cy, size, size, w, 1,
				  Scr.WhiteGC, Scr.BlackGC, SHADOW_ALL );
	DrawShadowBox( cx+1, cy+1, size-2, size-2, w, 1,
				  Scr.BlackGC, Scr.WhiteGC, SHADOW_ALL );
	XDrawLine( dpy, w, Scr.BlackGC, cx+3, cy+3, cx+size-4, cy+size-4 );
	XDrawLine( dpy, w, Scr.BlackGC, cx+size-4, cy+3, cx+3, cy+size-4 );
}
                     ...
                     const char *text, int active, GC fill_gc )
{
	...
	XFillRectangle( dpy, w, fill_gc, x, y, fw, fh );
	DrawShadowBox( x, y, fw, fh, w, 1,
				  Scr.WhiteGC, Scr.BlackGC, SHADOW_ALL );
	DrawShadowBox( x+1, y+1, fw-2, fh-2, w, 1,
				  Scr.BlackGC, Scr.WhiteGC, SHADOW_ALL );
	StrWidthHeight( WINDOWFONT, NULL, NULL, &text_off, "Mg", 2 );
	XDRAWSTRING( dpy, w, WINDOWFONT, Scr.BlackGC,
				x+4, y+fh/2-text_off+5, t, len );
	if( active ){
		StrWidthHeight( WINDOWFONT, &text_w, NULL, NULL, t, len );
		XDrawLine( dpy, w, Scr.BlackGC,
				  x+4+text_w+1, y+3, x+4+text_w+1, y+fh-3 );
	}
}
                     ...
                     const char *label, int active )
{
	...
	XFillRectangle( dpy, w, bg, x, y, fw, fh );
	DrawShadowBox( x, y, fw, fh, w, 1,
				  Scr.WhiteGC, Scr.BlackGC, SHADOW_ALL );
	DrawShadowBox( x+1, y+1, fw-2, fh-2, w, 1,
				  Scr.BlackGC, Scr.WhiteGC, SHADOW_ALL );
	StrWidthHeight( WINDOWFONT, &lw, &lh, &lo, l, len );
	XDRAWSTRING( dpy, w, WINDOWFONT, Scr.BlackGC,
				x+(fw-lw)/2, y+fh/2-lo+4, l, len );
}
