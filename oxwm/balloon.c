...
void BalloonSize( int *lh, int *lw, int *offset, int *w, int *h, OxwmWindow *mw )
{
	int lp;
...
void DrawWindowBalloon( int xoff, int yoff, int lh, int offset, int lw, OxwmWindow *mw )
{
	int lp;
	char contents[MAX_BALLOON_STR_LEN];
	int xinc, yinc;
...
void BalloonHelp( void )
{
	...
	if( XFindContext ( dpy, JunkChild, OxwmContext, (caddr_t *)&tmp_win )
	   == XCNOENT ){
		return;
	}
